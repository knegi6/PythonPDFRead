...
pfr.get_data_after_colon_social_security_number(pocr.get_text_from_any_pdf_into_list(pdf_file))
# pfr.find_social_security_number_line(pocr.get_text_from_any_pdf_into_list(pdf_file))
# pdfr.get_data_after_colon_agent_code
...
for index, row in df.iterrows():
    if row["STATUS"]=="PENDING":
        ...
        # print(file_list[0])
        print(row["PDF NAME"])
        agent_code_dict={"Path":"Social Security Number"}
        for file in file_list:
            
            try:
                # agent_code_dict[file]=pfr.get_policy_no_from_table(file)

                agent_code_dict[file]= pfr.get_data_after_colon_social_security_number(pocr.get_text_from_any_pdf_into_list(file))
